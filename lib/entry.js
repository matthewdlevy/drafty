  ...
  */
  function upsert(key, metadata, content, bucket, endpoint, callback) {
    async.waterfall([
      ...
      //get the current site structure to build the menu
      function(body, waterfallCb){
        $.ajax({
          url: SITE_ENDPOINT + DATA_KEY,
          dataType: 'json',
          success: function(structure){
            var menu = dodgercms.templates[MENU_TEMPLATE];
            var menu_html = menu({nav: structure});
            waterfallCb(null, body, menu_html);
          },
          error: function(){
            waterfallCb(new Error('Unable to retrieve site structure'));
          }
        });
      ...
      function(body, menu, waterfallCb) {
        ...
        if(typeof(metadata.template) != 'undefined') {
          ...
          entry = dodgercms.templates[DEFAULT_TEMPLATES[0].id];
        }
        var html = '<html></html>';
